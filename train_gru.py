...
def train_gru_model(data_dict, model_params, train_params, save_dir='models'):
    """
    Train GRU model for NILM
    ...
    # Create model
    model = GRUModel(input_size, hidden_size, num_layers, output_size, bidirectional)
    ...
    # Start training
    print(f"Starting GRU training on {device}...")
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(save_dir, f"gru_model_best.pth")
    ...
    # Save final model
    final_model_path = os.path.join(save_dir, f"gru_model_final.pth")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"gru_training_history.png"))
    plt.close()
    
    # Save training history to JSON
    with open(os.path.join(save_dir, 'gru_history.json'), 'w') as f:
        json_history = {
...
def train_gru_all_appliances(house_number=1, window_size=100, save_dir='models/gru'):
    """
    Train GRU model on all appliances in the specified house
    ...
    print(f"Training GRU models for {len(appliances)} appliances in house {house_number}:")
    ...
    # Train GRU model for each appliance
    for appliance_idx, appliance_name in appliances.items():
        print(f"\n{'-'*50}")
        print(f"Training GRU model for {appliance_name} (index {appliance_idx})")
        print(f"{'-'*50}\n")
        ...
        try:
            ...
            # Train the model
            model, history, best_model_path = train_gru_model(
                data_dict, 
                model_params, 
                train_params, 
            ...
            # Log success
            print(f"Successfully trained GRU model for {appliance_name}")
            
        except Exception as e:
            print(f"Error training GRU model for {appliance_name}: {str(e)}")
    ...
    print("\nGRU training completed for all appliances!")
    print(f"Results saved to {base_save_dir}")
    
    return results, base_save_dir

if __name__ == "__main__":
    # Train GRU models on all appliances in house 1
    results, save_dir = train_gru_all_appliances(house_number=1)