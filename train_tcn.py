...
def train_tcn_model(data_dict, model_params, train_params, save_dir='models'):
    """
    ...
    # Start training
    print(f"Starting TCN training on {device}...")
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(save_dir, f"tcn_model_best.pth")
    ...
    # Save final model
    final_model_path = os.path.join(save_dir, f"tcn_model_final.pth")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"tcn_training_history.png"))
    plt.close()
    
    # Save training history to JSON
    with open(os.path.join(save_dir, 'tcn_history.json'), 'w') as f:
        json_history = {
...
def train_tcn_all_appliances(house_number=1, window_size=100, save_dir='models/tcn'):
    """
    ...
    print(f"Training TCN models for {len(appliances)} appliances in house {house_number}:")
    ...
    for appliance_idx, appliance_name in appliances.items():
        print(f"\n{'-'*50}")
        print(f"Training TCN model for {appliance_name} (index {appliance_idx})")
        print(f"{'-'*50}\n")
        ...
        try:
            ...
            # Train the model
            model, history, best_model_path = train_tcn_model(
                data_dict, 
                model_params, 
                train_params, 
            ...
            # Log success
            print(f"Successfully trained TCN model for {appliance_name}")
            
        except Exception as e:
            print(f"Error training TCN model for {appliance_name}: {str(e)}")
    ...
    print("\nTCN training completed for all appliances!")
    print(f"Results saved to {base_save_dir}")
    
    return results, base_save_dir

if __name__ == "__main__":
    # Train TCN models on all appliances in house 1
    results, save_dir = train_tcn_all_appliances(house_number=1)