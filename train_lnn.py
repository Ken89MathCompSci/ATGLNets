...
def train_lnn_model(data_dict, model_params, train_params, save_dir='models'):
    """
    Train standard Liquid Neural Network model for NILM.
    ...
    print(f"Starting LNN training on {device}...")
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            counter         = 0
            best_model_path = os.path.join(save_dir, "lnn_model_best.pth")
    ...
    final_path = os.path.join(save_dir, "lnn_model_final.pth")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, "lnn_training_history.png"))
    plt.close()

    with open(os.path.join(save_dir, 'lnn_history.json'), 'w') as f:
        json.dump({
...
def train_lnn_all_appliances(house_number=1, window_size=100, save_dir='models/lnn'):
    """
    Train standard LNN on all target appliances for the specified house.
    """
    ...
    print(f"Training LNN for {len(appliances)} appliances in house {house_number}:")
    ...
    for appliance_idx, appliance_name in appliances.items():
        ...
        try:
            ...
            model, history, best_path = train_lnn_model(
                data_dict,
                model_params={'input_size': 1, 'hidden_size': 128, 'output_size': 1, 'dt': 0.1},
    ...
    summary = {
        'timestamp': timestamp, 'house_number': house_number, 'model': 'lnn',
        'results': {
    ...
    print(f"\nLNN training complete. Results saved to {base_save_dir}")
    return results, base_save_dir


if __name__ == "__main__":
    results, save_dir = train_lnn_all_appliances(house_number=1)
