...
def train_lnn_model(data_dict, model_params, train_params, save_dir='models'):
    ...
    print(f"Starting LNN training for '{appliance_name}' on {device}...")
    ...
    for epoch in range(epochs):
        ...
        if not np.isnan(avg_val_loss) and avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(
                save_dir,
                f"lnn_{appliance_name.replace(' ', '_')}_best.pth")
    ...
    plt.savefig(os.path.join(
        save_dir,
        f"lnn_{appliance_name.replace(' ', '_')}_metrics.png"),
    ...
    with open(os.path.join(
            save_dir,
            f"lnn_{appliance_name.replace(' ', '_')}_history.json"),
...
def train_lnn_all_appliances(house_number=2, window_size=100, save_dir='models/lnn'):
    """
    ...
    for appliance_idx, appliance_name in appliances.items():
        ...
        try:
            ...
            model, history, test_metrics, best_path = train_lnn_model(
                data_dict,
                model_params={'input_size': 1, 'hidden_size': 64,
                              'output_size': 1, 'dt': 0.1},
...
if __name__ == "__main__":
    results, save_dir = train_lnn_all_appliances(house_number=2)
