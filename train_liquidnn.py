...
def train_liquid_model(data_dict, model_params, train_params, save_dir='models', advanced=True):
    """
    Train Liquid Neural Network model for NILM
    ...
    Args:
        ...
        advanced: Whether to use the advanced liquid network model
    ...
    if advanced:
        ...
        model = LiquidNetworkModel(input_size, hidden_size, output_size, dt)
        model_name = "liquid"
    ...
    # Save final model
    final_model_path = os.path.join(save_dir, f"{model_name}_model_final.pth")
...
def train_liquid_all_appliances(house_number=1, window_size=100, save_dir='models/liquid', advanced=True):
    """
    Train Liquid Neural Network model on all appliances in the specified house
    ...
    print(f"Training {model_type} models for {len(appliances)} appliances in house {house_number}:")
    ...
    # Train Liquid Network model for each appliance
    for appliance_idx, appliance_name in appliances.items():
        print(f"\n{'-'*50}")
        print(f"Training {model_type} model for {appliance_name} (index {appliance_idx})")
        print(f"{'-'*50}\n")
        ...
        try:
            ...
            # Train the model
            model, history, best_model_path = train_liquid_model(
                data_dict, 
                model_params, 
                train_params, 
            ...
            # Log success
            print(f"Successfully trained {model_type} model for {appliance_name}")
            
        except Exception as e:
            print(f"Error training {model_type} model for {appliance_name}: {str(e)}")
    ...
    print(f"\n{model_type} training completed for all appliances!")
    print(f"Results saved to {base_save_dir}")
    
    return results, base_save_dir

if __name__ == "__main__":
    # Train standard Liquid Neural Network models on all appliances in house 1
    results, save_dir = train_liquid_all_appliances(house_number=1, advanced=False)

    # Train Advanced Liquid Neural Network models on all appliances in house 1
    results, save_dir = train_liquid_all_appliances(house_number=1, advanced=True)