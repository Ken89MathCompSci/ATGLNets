...
def train_resnet_model(data_dict, model_params, train_params, save_dir='models'):
    """
    Train ResNet model for NILM
    ...
    layers = model_params.get('layers', [2, 2, 2])
    base_width = model_params.get('base_width', 32)
    
    # Create model
    model = ResNetModel(input_size, output_size, layers, base_width)
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            # Save best model
            best_model_path = os.path.join(save_dir, f"resnet_model_best.pth")
    ...
    # Save final model
    final_model_path = os.path.join(save_dir, f"resnet_model_final.pth")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"resnet_training_history.png"))
    plt.close()
...
def train_resnet_all_appliances(house_number=1, window_size=100, save_dir='models/resnet'):
    """
    Train ResNet model on all appliances in the specified house
    ...
    # Train ResNet model for each appliance
    for appliance_idx, appliance_name in appliances.items():
        print(f"\n{'-'*50}")
        print(f"Training ResNet model for {appliance_name} (index {appliance_idx})")
        print(f"{'-'*50}\n")
        ...
        try:
            ...
            model_params = {
                ...
                'layers': [2, 2, 2],          # Number of residual blocks in each layer
                'base_width': 32              # Base width for the network
            }
            ...
            # Train the model
            model, history, best_model_path = train_resnet_model(
                data_dict, 
                model_params, 
                train_params, 
            ...
            # Log success
            print(f"Successfully trained ResNet model for {appliance_name}")
            
        except Exception as e:
            print(f"Error training ResNet model for {appliance_name}: {str(e)}")
    ...
    print("\nResNet training completed for all appliances!")
